...
class TestResPartnerOperatingUnit(OperatingUnitCommon):
    ...
    def test_create_user_partner_already_has_ou_raises_error(self):
        """Creating a user whose partner already has mismatching OUs
        must raise UserError."""
        # Create a standalone partner with ou1
        ...
        # Attempt to create a user linked to that partner with a different OU (b2c)
        with self.assertRaises(UserError):
            self.res_users_model.create(
                {
