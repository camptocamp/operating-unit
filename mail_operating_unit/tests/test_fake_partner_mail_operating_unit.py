...
class MailOperatingUnitCommon(common.TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
    ...
    def setUp(self):
        super().setUp()
        self.fake_partner = self.env["res.partner"].create({"name": "Fake Partner"})
        self.company = self.env.ref("base.main_company")
        self.operating_unit_alias_domain = self.env["mail.alias.domain"].create(
        ...
        self.operating_unit = self.env["operating.unit"].create(
            {
                ...
                "alias_domain_id": self.operating_unit_alias_domain.id,
            }
        )
