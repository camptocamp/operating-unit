...
class TestFakePartnerMailOperatingUnit(FakePartnerMailOperatingUnitCommon):
    ...
    def test_05_template_mail_server_keeps_priority(self):
        ...
        self._set_ou_mail_server_enabled(True)
    ...
    def test_09_record_operating_unit_mail_server_feature_disabled(self):
        """Do not apply OU mail server routing when the feature is disabled."""
        self._set_ou_mail_server_enabled(False)
        self.mail_template.write({"mail_server_id": False})
        self.fake_partner.write(
            {
                "operating_unit_id": False,
        ...
        with mock.patch.object(
            type(self.mail_template),
            "_get_operating_unit_mail_server",
        ) as mocked:
            self.mail_template.send_mail(self.fake_partner.id, force_send=False)
