...
Unit and, when enabled in General Settings, automatically route emails
through that server when no mail server is explicitly defined on the
email template.
...
- Enable *Use operating unit outgoing mail server* in *General Settings*
  if outgoing mail servers should be selected automatically from the
  Operating Unit.
...
When the outgoing mail server feature is disabled, or when no
unambiguous Operating Unit mail server can be determined, the standard
Odoo outgoing mail server selection is applied.
