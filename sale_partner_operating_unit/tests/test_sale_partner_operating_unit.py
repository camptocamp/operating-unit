...
class TestSalePartnerOperatingUnit(TestSaleOperatingUnit):
    def test_sale_partner_operating_unit(self):
        ...
        # Check sale order has no OU
        self.assertEqual(self.sale1.operating_unit_id, False)
        # Assign the team to the sale order
        self.sale1.write({"team_id": self.sale_team_ou2.id})
        # Check sale order is of the teams' OU
        self.assertEqual(
            self.sale1.operating_unit_id, self.sale_team_ou2.operating_unit_id
        )
